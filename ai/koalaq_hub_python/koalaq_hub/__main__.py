...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f"koalaq_{datetime.now().strftime('%Y%m%d')}.log")
...
def validate_and_print_config():
    ...
    for agent_id in available_agents:
        ...
        # 检查主LLM配置
        if app.llm_use:
            llm = llm_builder.get_llm(app.llm_use)
            if not llm:
        ...
        if app.enable_summary and app.summary_llm_use:
            summary_llm = llm_builder.get_llm(app.summary_llm_use)
            if not summary_llm:
                main_logger.error(
...
def initialize_model_data(db_storage: RepositoryAdapter):
    """初始化模型数据到数据库"""
    try:
        main_logger.info("初始化模型数据...")

        # 获取所有LLM对象
        all_llms = llm_builder.get_all_llms()

        for llm_id, llm in all_llms.items():
            platform = llm.platform
            model = llm.model

            # 创建或更新模型记录
            db_storage.create_or_update_model(platform, model)
            main_logger.info(
                "初始化模型记录",
                {"platform": platform, "model": model, "llm_id": llm_id},
            )

        main_logger.info("模型数据初始化完成")

    except Exception as e:
        main_logger.error("初始化模型数据失败", exception=e)
        raise
...
async def initialize_services():
    ...
    try:
        ...
        await server_manager.start_periodic_refresh()
        ...
        # 初始化模型数据
        if db_storage:
            initialize_model_data(db_storage)
...
async def main() -> None:
    ...
    try:
        ...
        if not all_llms:
            ...
            return
        main_logger.info(f"已加载 {len(all_llms)} 个 LLM 配置: {list(all_llms.keys())}")
