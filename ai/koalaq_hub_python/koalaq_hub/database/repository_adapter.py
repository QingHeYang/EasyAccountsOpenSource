...
class RepositoryAdapter:
    ...
    def create_or_update_model(self, platform: str, model: str) -> int:
        """创建或更新模型"""
        return self.factory.get_token_repository().create_or_update_model(platform, model)

    def add_tokens_to_model(self, platform: str, model: str, token_usage: TokenUsage):
        """为模型累加Token
        ...
        """
        total_tokens, prompt_tokens, completion_tokens, reasoning_tokens = self._extract_token_values(token_usage)
        return self.factory.get_token_repository().add_tokens_to_model(
            platform, model, total_tokens, prompt_tokens, completion_tokens, reasoning_tokens
        )
