...
from ..tool.function.inner_tool_manager import InnerToolManager
...
class AgentRegistry:
    ...
    def __init__(self, tool_manager: Optional[ToolManager] = None, 
        ...
        # 内部工具管理器
        self.inner_tool_manager = InnerToolManager()
    ...
    def create_agent_from_config(self, agent_id: str) -> Optional['Agent']:
        ...
        agent = Agent(
            ...
            # ReAct模式配置
            enable_react_mode=config.enable_react_mode,
            react_style=config.react_style,
            # 提示词组装配置
            base_modules=list(config.base_modules),  # 复制列表
    ...
    def build_sub_agent(self, agent_id: str) -> Optional['Agent']:
        ...
        sub_agent = Agent(
            ...
            # 提示词组装配置
            base_modules=list(config.base_modules),
            task_instructions_file=config.task_instructions_file,
    ...
    def build_tools_list(self, agent: Agent) -> None:
        ...
        if config.tool_mode == "function_calling":
            ...
            if self.tool_manager:
                mcp_tools = self.tool_manager.get_tools_for_application(agent)
                # 转换为Function Calling格式
                tools.extend(ToolConverter.mcp_list_to_function_format(mcp_tools))
            
            # 2. 获取内部工具列表（如ask_agent）
            if self.inner_tool_manager:
                inner_tools = self.inner_tool_manager.get_tools_for_agent(agent)
    ...
    def build_system_prompt(self, agent: Agent, user_id: str, conversation_id: str) -> str:
        ...
        Returns:
            组装好的系统提示词
        """
        # 构建Function Calling工具列表
        self.build_tools_list(agent)
