...
def create_users_router(user_manager):
    ...
    async def create_user(user_input: UserInput):
        ...
        try:
            ...
            return ResponseBuilder.created(data=user.dict(), message="用户创建成功")
            ...
            return JSONResponse(status_code=500, content=ResponseBuilder.internal_error("创建用户失败").dict())
    ...
    async def list_users():
        ...
        try:
            ...
            return JSONResponse(status_code=500, content=ResponseBuilder.internal_error("获取用户列表失败").dict())
    ...
    async def get_current_user_profile(request: Request):
        ...
        try:
            ...
            return JSONResponse(status_code=500, content=ResponseBuilder.internal_error("获取用户信息失败").dict())
    ...
    async def get_user(user_id: str):
        ...
        try:
            ...
            if not user:
                logger.warning("用户不存在", {"user_id": user_id})
                return JSONResponse(status_code=404, content=ResponseBuilder.not_found("用户不存在").dict())

            logger.info("获取用户成功", {"user_id": user_id})
            return ResponseBuilder.success(data=user.dict(), message="获取用户成功")
        except Exception as e:
            logger.error("获取用户失败", exception=e, extra_data={"user_id": user_id})
            return JSONResponse(status_code=500, content=ResponseBuilder.internal_error("获取用户失败").dict())
