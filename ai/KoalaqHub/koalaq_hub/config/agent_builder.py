...
class AgentConfig:
    ...
    def inner_tools(self) -> List[str]:
        """获取内部工具列表"""
        inner_tools_str = self._data.get("inner_tools", "[]")
        try:
            tools_list = json.loads(inner_tools_str)
            return tools_list if isinstance(tools_list, list) else []
        except Exception as e:
            print(f"解析内部工具列表失败: {e}")
            return []
