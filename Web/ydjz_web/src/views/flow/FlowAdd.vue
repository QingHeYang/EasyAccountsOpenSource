<template>
  <div style="width:100vw; height:100vh; background: #F7F8FA">
    ...
    <van-cell-group>
      <!-- 新增明细编号显示  v2.5版本 -->
      <van-cell v-if="$route.query.flowId" title="明细编号" :value="'#' + $route.query.flowId"
        :value-style="{ color: '#999' }" />

      <van-field input-align="right" v-model="money" type="number" label="明细金额" placeholder="请输入明细金额" />
      ...
      <!-- 2.5.0 新增关联明细选项 -->
      <van-cell title="关联明细" is-link @click="onRelatedClick">
        <template #value>
          <van-tag v-if="relatedFlow" :type="relatedFlow.tagStyle">#{{ relatedFlow.linkId }}</van-tag>
        </template>
      </van-cell>

      <van-field v-model="note" rows="2" autosize label="备注" type="textarea" maxlength="50" placeholder="请输入备注"
        show-word-limit input-align="right" />
    </van-cell-group>
    ...
    <van-popup v-model:show="typeCascaderShow" round position="bottom">
      <van-cascader v-model="cascaderValue" title="选择明细分类" :options="allTypes" active-color="#1989fa"
    ...
    <van-cell-group v-for="child in childMoneyItem" :key="child.index" inset
      ...
      <van-swipe-cell>
        <van-field input-align="left" v-model="child.money" type="number" label="明细金额" placeholder="请输入追加明细金额" />
    ...
    <div style="margin: 16px">
      <van-button v-if="$route.query.flowId" @click="doAddNewItemMoney" style="margin-bottom: 5px" round block
        type="warning" native-type="submit">
        追加分明细
      </van-button>

      <!-- 调整按钮顺序，将删除按钮放在修改按钮左边 -->
      <div v-if="$route.query.flowId" style="display: flex; gap: 10px">
        <van-button @click="onDeleteBtnClick" round block type="danger">删除</van-button>
        <van-button @click="onSubmitBtnClick" round block type="primary">修改</van-button>
      </div>
      <van-button v-else @click="onSubmitBtnClick" round block type="primary">提交</van-button>
    </div>

    <!--  快记模板 2.1版本  -->
    <van-popup v-model:show="fastPopupShow" position="top"
    ...
    <van-dialog v-model:show="fastDialogShow" closeOnClickOverlay cancel-button-text="编辑" confirm-button-text="选择"
      ...
      @confirm="fastChooseClick(chooseTemplate)" show-cancel-button>
...
export default {
  ...
  methods: {
    ...
    dealRelatedFlow() {
      const selectedRelatedFlow = localStorage.getItem('selectedRelatedFlow');
      if (selectedRelatedFlow) {
        this.relatedFlow = JSON.parse(selectedRelatedFlow)
        if (this.relatedFlow.handle == 0) {
          this.relatedFlow.tagStyle = "success"
        } else if (this.relatedFlow.handle == 1) {
          this.relatedFlow.tagStyle = "danger"
        } else {
          this.relatedFlow.tagStyle = "primary"
        }
      }
    },
    ...
    doGetCurrentFlow() {
      this.$http({
        ...
        if (this.relatedFlow == null) {
          this.relatedFlow = flow.relatedFlow
        }
        this.doGetTypes()
      })
    },
    ...
    onRelatedClick() {
      this.$router.push({ path: "/screen/choose" });
    },
    ...
    onDeleteBtnClick() {
      showConfirmDialog({
        ...
        .then(() => {
          this.$http({
            url: "/flow/deleteFlow/" + this.$route.query.flowId,
            method: "delete"
          }).then(() => {
