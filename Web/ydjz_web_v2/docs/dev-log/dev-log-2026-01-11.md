...
#### 8.1 收支列表页面（Action.vue）

添加 `getExemptText` 函数，根据收支类型和不计入模式显示不同文本：
...
#### 8.2 收支添加/编辑页面（ActionAdd.vue）

**新增功能：**
- 导入 `ExemptMode` 枚举
- 添加 `exemptMode` 状态管理
- 双向计算属性 `fromNotCount` 和 `toNotCount`
- 转账不计入设置卡片UI
...
```vue
<div v-if="!isLiabilityAccount && accountForm.exemptMoney" class="net-asset-card">
  <div class="net-asset-formula">账户净资产 = 账户余额 - 不计入金额</div>
  <div class="net-asset-calc">
    <span class="net-asset-value">{{ netAsset }}</span>
    <span class="calc-equal">=</span>
    <span class="calc-item">({{ accountForm.money || '0.00' }})</span>
    <span class="calc-operator">-</span>
    <span class="calc-item">({{ accountForm.exemptMoney || '0.00' }})</span>
  </div>
</div>
```
...
| 文件路径 | 修改内容 |
|---------|---------|
| `src/mobile/views/setting/Action.vue` | 添加 ExemptMode 导入、getExemptText 函数 |
| `src/mobile/views/setting/ActionAdd.vue` | ExemptMode 支持、双向计算属性、转账卡片UI、帮助对话框 |
| `src/mobile/views/setting/Account.vue` | AccountType 标签、不计入金额显示、预览对话框美化、停用警告 |
| `src/mobile/views/setting/AccountAdd.vue` | 账户类型选择、不计入金额、净资产计算、帮助对话框 |
...
| 正确术语 | 避免使用 | 说明 |
|---------|---------|------|
| 不计入 | 豁免、免除 | 指不计入总金额的金额 |
| 不计入金额 | 豁免金额 | 账户中不计入净资产的部分 |
| 不计入总金额 | 豁免模式 | 收支类型的设置选项 |

代码中仍使用 `exempt`、`exemptMoney`、`ExemptMode` 等英文命名，但用户界面全部使用中文"不计入"表述。

---

## 备注

本日开发完成了桌面端和移动端的账户类型、不计入金额功能。所有 API 类型定义已在 `shared/api` 中统一，两端共用。

**已完成：**
- ✅ 桌面端账户管理（AccountManager.vue）
- ✅ 桌面端收支管理（ActionManager.vue）
- ✅ 桌面端流水编辑器账户选择面板优化
- ✅ 移动端收支管理（Action.vue、ActionAdd.vue）
- ✅ 移动端账户管理（Account.vue、AccountAdd.vue）

**待确认：**
- 移动端记账页面（FlowAdd.vue）是否需要根据账户类型添加限制逻辑
- 统计页面净资产计算是否需要调整
