...
async def handle_webhook(file: UploadFile = File(...), file_name: str = Form(...), file_type: str = Form(...)):
    ...
    # v2.7.0: scheduled_reminder 无全局开关——发不发由规则 emailEnabled 决定（Server 端控制）
...
async def send_email_with_attachment(file_content: bytes, file_name: str, file_type: str):
    ...
    elif file_type == "scheduled_reminder":
        # v2.7.0: 定时记账提醒邮件
        # 约定：file_name = 邮件主题；file_content（文本字节）= 邮件正文；不带附件
        # 是否发由 Server 端规则 emailEnabled 控制，到这里说明一定要发
        msg['Subject'] = file_name
        try:
            body_text = file_content.decode('utf-8')
        except UnicodeDecodeError:
            body_text = file_content.decode('utf-8', errors='replace')
    ...
    if body is None:
        ...
        logger.info(f"邮件 '{msg['Subject']}' 已发送到 {recipient_list}")
        return f"邮件 '{msg['Subject']}' 已发送到 {recipient_list}，发送结果：OK"
